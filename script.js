...
module.exports = async (cid) => {
  ...
  let startDate = new Date('Jan 1, 2014');
  ...
  if (channelCreate) {
    let date = /(\w+ \d+, \d+)"/gm.exec(channelCreate);
    let cusername = /{"channelMetadataRenderer":{"title":"(?<=\")(.*?)(?=\")","description"/gm.exec(channelCreate);
  ...
  if (verify?.status == 200) {
    ...
    for (i = endDate; i >= startDate; i -= gap) {
      ...
      if (res?.status == 200) {
        ...
        console.log(false, res?.status, res?.statusText);
        break;
      }
    }
  } else {
